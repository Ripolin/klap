...
func init() {
	...
	utilruntime.Must(klapv1alpha1.AddToScheme(scheme))
...
func main() {
	...
	mgr, err := ctrl.NewManager(ctrl.GetConfigOrDie(), ctrl.Options{
		...
		LeaderElectionID:       "2f62bd50.my.domain",
		...
		// Do not cache secrets, as they may contain sensitive information that should not be
		// stored in memory for long periods of time. Also keep memory usage low by not caching
		// secrets, as they may be large and numerous.
	...
	if err := (&controller.EntryReconciler{
	...
	if os.Getenv("ENABLE_WEBHOOKS") != "false" {
		if err := webhookv1alpha1.SetupEntryWebhookWithManager(mgr); err != nil {
			setupLog.Error(err, "Failed to create webhook", "webhook", "Entry")
			os.Exit(1)
		}
	}
	// nolint:goconst
